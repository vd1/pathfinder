...
\input{assets/2026-09-16-200514/numbers.tex}
\newcommand{\kicker}[1]{{\color{Muted}\fontsize{8}{10}\selectfont\MakeUppercase{#1}\par}\vspace{2mm}}
\newcommand{\smallprint}[1]{\par{\color{Muted}\fontsize{7}{9}\selectfont #1\par}}
\newcommand{\takeaway}[1]{\begin{beamercolorbox}[wd=\linewidth,sep=6pt]{conclusion}
  \fontsize{10.5}{13}\selectfont #1\end{beamercolorbox}}
\newcommand{\papercard}[5][CardFill]{\begin{tikzpicture}
  \node[draw=Teal,fill=#1,line width=0.6pt,rounded corners=4pt,
    inner sep=5pt,outer sep=0pt,
    text width=\dimexpr\linewidth-10pt-0.6pt\relax,align=left] {%
      \begin{minipage}[t][36pt][t]{\linewidth}
      {\color{Teal}\fontsize{8.7}{10}\selectfont\bfseries #2\enspace #3\par}\vspace{1pt}
      {\color{Ink}\fontsize{8.2}{10}\selectfont #4\par #5\par}
      \end{minipage}};
  \end{tikzpicture}}

% mini abstracts
\newcommand{\Qfour}{\papercard{Q4}{Incentives to report what one knows}
  {Agents may profit by hiding what they know.}
  {Rewards can make truthful reporting worthwhile.}}
\newcommand{\Psix}{\papercard[PCardFill]{P6}{An outside judge for a team}
  {A team works together; credit is hard to assign.}
  {An outside AI judges contributions from images.}}
\newcommand{\Qseven}{\papercard{Q7}{Sharing resources}
  {Participants compete for limited resources.}
  {Messages and payments aim to share them well.}}
\newcommand{\Pone}{\papercard[PCardFill]{P1}{Learning about bad outcomes}
  {Learn to act while controlling bad outcomes.}
  {Estimate losses in the worst fraction of cases.}}
\newcommand{\Qthree}{\papercard{Q3}{AI agents in a market}
  {AI agents buy and sell in a simulated market.}
  {Their performance is compared with humans.}}
\newcommand{\Pthree}{\papercard[PCardFill]{P3}{A critic for the code reviewer}
...
\begin{frame}[plain]
\kicker{IQ lab, LIP6\quad /\quad 17 September 2026}
\vspace{8mm}
{\rmfamily\fontsize{46}{50}\selectfont Pathfinder}\par\vspace{3mm}
{\rmfamily\fontsize{24}{29}\selectfont From seminar questions to research}\par
\vspace{5mm}{\color{Teal}\rule{28mm}{1.5pt}}\par\vspace{10mm}
\begin{columns}[T,onlytextwidth]
  \begin{column}{.31\textwidth}
    {\large\bfseries Vincent Danos}\par\vspace{2mm}
    {\color{Muted}CNRS, ENS}
  \end{column}
  \begin{column}{.34\textwidth}
    {\large\bfseries Elham Kashefi}\par\vspace{2mm}
    {\color{Muted}CNRS, LIP6\\University of Edinburgh}
  \end{column}
  \begin{column}{.31\textwidth}
    {\large\bfseries William Waites}\par\vspace{2mm}
    {\color{Muted}University of Edinburgh}
  \end{column}
\end{columns}
\vspace{12mm}
\href{https://github.com/vd1/pathfinder}{github.com/vd1/pathfinder}
...
\vspace{2mm}
\paperpair{\smallprint{Mechanism design}}{\smallprint{Systems of cooperating agents}}
...
\kicker{Pathfinder: from seminar questions to research}
\begin{tikzpicture}[x=1.22cm,y=1cm]
  \path[use as bounding box] (0,0) rectangle (14.5,3.85);
  \fill[Mist] (.15,.2) rectangle (4.2,3.7);
  \draw[Teal,line width=1pt] (.65,1.7) rectangle (3.7,3.2);
  \node[font=\small,align=center] at (2.175,2.45) {An outside judge\\ranks the agents};
  \fill[Teal] (1.25,1.18) circle (.17);
  \draw[Teal,line width=2pt] (1.25,1) -- (1.25,.5);
  \draw[Teal,line width=1pt] (1.25,.8) -- (1.8,1.3);
  \node[anchor=west,font=\scriptsize,text=Muted] at (.45,.35) {P6's author Alice explains P6};
  \node[anchor=north west,text width=9.4cm,align=left,font=\large] at (4.65,3.55)
    {Bob: ``Why use a judge? Could you not ask the agents directly?''};
  \node[anchor=north west,text width=9cm,align=left,font=\large,text=Rust] at (5.25,2.4)
    {Alice: ``We could \ldots but wouldn't they misreport?''};
  \node[anchor=north west,text width=9cm,align=left,font=\large] at (4.65,1.25)
    {Bob: ``Glad you ask, I have just heard of a mechanism-design paper about that\ldots''};
  \node[anchor=north west,text width=9cm,align=left,font=\large,text=Rust] at (5.25,0.1)
    {Alice: ``Interesting!''};
\end{tikzpicture}
\vspace{5mm}
...
\begin{column}{.61\textwidth}
  \resizebox{8.5cm}{!}{\input{assets/2026-09-16-200514/pilot-heatmap.tex}}
\end{column}
\begin{column}{.36\textwidth}
  {\rmfamily\fontsize{32}{34}\selectfont 100}\quad possible encounters\par\vspace{3mm}
  {\rmfamily\fontsize{25}{28}\selectfont 14}\quad investigated pairs\par\vspace{3mm}
  {\rmfamily\fontsize{25}{28}\selectfont 5}\quad accepted manuscripts\par\vspace{4mm}
  {\small Colour: feasibility $\times$ gain.\par
  Outline: investigated.\par Star: internally accepted.}\vspace{3mm}
...
% \kicker{A familiar scientific process, made explicit}
\begin{tikzpicture}[x=1.22cm,y=1cm]
  \path[use as bounding box] (0,-.3) rectangle (14.5,4.6);
  \node[state,text width=3.2cm,minimum height=1.2cm] (scan) at (1.85,3.6)
    {\Large SCAN\\\small Propose and prioritise questions};
  \node[state,text width=3.3cm,minimum height=1.2cm] (research) at (6.3,3.6)
    {\Large RESEARCH\\\small Investigate; challenge; consolidate};
  \node[state,text width=3.6cm,minimum height=1.2cm] (edit) at (12.2,3.6)
    {\Large EDIT\\\small Make the outcome assessable};
  \draw[flow] (scan) -- node[lab,above] {selected\\question} (research);
  \draw[flow] (research) -- node[lab,above] {terminal\\outcome} (edit);
  \node[paused,text width=4.4cm,anchor=west] (account) at ([yshift=-2.05cm]edit.west)
    {Readable account\\\scriptsize Human-centric explanation};
  \node[state,fill=green!10!Paper,text width=4.4cm,anchor=west] (paper) at ([yshift=-2.05cm]research.west)
    {Publication manuscript\\\scriptsize Author $\rightleftarrows$ Reviewer};
  \coordinate (editbranch) at ([yshift=-3mm]edit.south);
  \draw[draw=Teal,line width=.8pt] (edit.south) -- (editbranch);
  \draw[flow] (editbranch) -| node[lab,above,pos=.25] {DRAFT} (paper.north);
  \draw[flow] (editbranch) -- node[lab,right,xshift=2mm] {PAUSE} (editbranch |- account.north);
  % \node[anchor=west,font=\small,text=Muted] at (.2,.1)
  %   {The readable account explains the result; it does not change the research verdict.};
\end{tikzpicture}
\takeaway{The output may be a result, an obstruction, a mistake in either papers, or an account of what is missing to pursue the connexion.}
...
% \kicker{Recover completed work. Account for resources. Reuse context.}
\vspace{3mm}
\begin{columns}[T,onlytextwidth]
  \begin{column}{.315\textwidth}
    {\Large\color{Teal}\textbf{Recover}}\par\vspace{3mm}
    Keep artifacts and the ledger.\par\vspace{2mm}
    Resume from the recorded stage.\par\vspace{2mm}
    Retry failed calls within limits.\par\vspace{3mm}
    \smallprint{A transport failure is not a scientific PAUSE.}
  \end{column}
  \begin{column}{.315\textwidth}
    {\Large\color{Teal}\textbf{Account}}\par\vspace{3mm}
    Record tokens and elapsed time per call.\par\vspace{2mm}
    Bound calls, rounds and time.\par\vspace{2mm}
    Check budget before admitting work.\par\vspace{3mm}
    \smallprint{An admission guard is not a hard cap on the final bill.}
  \end{column}
  \begin{column}{.315\textwidth}
    {\Large\color{Teal}\textbf{Reuse}}\par\vspace{3mm}
    Keep common context at the front.\par\vspace{2mm}
    Append new ledger entries.\par\vspace{2mm}
    Put changing instructions last.\par\vspace{3mm}
    \smallprint{Identical prefixes can be cached across calls.}
  \end{column}
\end{columns}
\vspace{6mm}
\begin{tikzpicture}[x=1cm,y=1cm]
  \node[anchor=west,text=Muted,font=\small,inner sep=0pt]
    at (0,1.05) {Prompt context, left to right};
  \fill[CardFill] (0,0) rectangle (5.8,.75);
  \fill[Mist] (5.8,0) rectangle (11.6,.75);
  \fill[Warm] (11.6,0) rectangle (17.4,.75);
  \draw[Teal,line width=.6pt] (0,0) rectangle (17.4,.75);
  \draw[Teal,line width=.6pt] (5.8,0) -- (5.8,.75) (11.6,0) -- (11.6,.75);
  \node[text=Ink,font=\small] at (2.9,.375) {Shared papers};
  \node[text=Ink,font=\small] at (8.7,.375) {Growing ledger};
  \node[text=Ink,font=\small] at (14.5,.375) {Current call's instructions};
\end{tikzpicture}
\vspace{7mm}
\takeaway{Persistent state supports recovery; resource records support control; stable prefixes reduce repeated input cost.}
...
\paperpair{\Qfour}{\Psix}
\begin{tikzpicture}[x=1.22cm,y=1cm]
  \path[use as bounding box] (0,0) rectangle (14.5,3.3);
  \node[font=\small\bfseries,anchor=west] at (.2,3.05) {SEED};
  \node[font=\small\bfseries,text=Rust,anchor=west] at (4.9,3.05) {OBSTACLE};
  \node[font=\small\bfseries,text=Teal,anchor=west] at (10,3.05) {CONDITIONAL RESULT};
  \node[align=left,text width=3.8cm,anchor=north west,font=\small] at (.2,2.65)
    {Replace judge with participants' reports.};
  \node[align=left,text width=4.3cm,anchor=north west,font=\small] at (4.9,2.65)
    {Truthful reports can still\\be insufficient for ranking.};
  \draw[flow] (4.15,2.3) -- (4.7,2.3);
  \draw[flow] (9.15,2.3) -- (9.75,2.3);
  \node[agent] (a) at (10.65,1.7) {};
  \node[agent] (b) at (11.75,2.35) {};
  \node[agent] (c) at (11.75,1.05) {};
  \node[agent] (d) at (13,2.35) {};
  \node[agent] (e) at (13.65,1.05) {};
  \draw[Teal,line width=1pt] (a)--(b)--(c)--(a) (b)--(d)--(e)--(c);
  \node[font=\scriptsize,align=center] at (12.1,.45) {overlap calibrates observers\\and connects the ranking};
  \node[anchor=west,text width=8.5cm,font=\small,align=left] at (.2,.8)
...
\begin{tikzpicture}[x=1.22cm,y=1cm]
  ...
  \node[font=\small\bfseries,anchor=west] at (.15,2.95) {FIRST RUN};
  ...
  \node[font=\small\bfseries,text=Rust,anchor=west] at (.15,1.15) {RERUN};
...
\begin{columns}[T,onlytextwidth]
  \begin{column}{.315\textwidth}
    {\color{Teal}\textbf{SEED: spot a rush to finish}}\par\vspace{2mm}
  ...
  \end{column}
  \begin{column}{.315\textwidth}
    {\color{Teal}\textbf{SHIFT: test simpler agents}}\par\vspace{2mm}
  ...
  \end{column}
  \begin{column}{.315\textwidth}
    {\color{Rust}\textbf{TEST: compare fairly}}\par\vspace{2mm}
...
\begin{tikzpicture}[x=1.22cm,y=1cm]
  ...
  \foreach \label/\value/\y in {Scan/3.74692/2.7,Research/410.56226/1.9,Manuscript/116.11062/1.1,Account/70.52111/.3}{
    ...
    \fill[Teal] (2.1,\y-.18) rectangle (2.1+\barw,\y+.18);
...
\begin{column}{.52\textwidth}
  \centering
  \includegraphics[width=\linewidth,height=6.1cm,keepaspectratio]{\assetdir/julien-dashboard.png}
\end{column}
\begin{column}{.45\textwidth}
  ...
  \textbf{98} finished threads\par\vspace{1mm}
  ...
  {\small The expert already knows\\one side of the connection.}\par\vspace{2mm}
  \smallprint{Expert assessment becomes a search for directions in their own programme.}
\end{column}
\end{columns}
\vspace{2mm}\takeaway{\textbf{Human validation:} do the notes add something new and useful to the expert's research? Internal verdicts are not enough.}
...
Expert judgment complements proofs and experiments; it does not replace them.
...
\begin{column}{.62\textwidth}
  ...
  \hspace*{8mm}\includegraphics[width=10.6cm,height=6.7cm,keepaspectratio]{\assetdir/qsl-dashboard.pdf}
  % \smallprint{\QSLRows\ QSL papers (rows). Frozen paper-pair universe.}
\end{column}
\begin{column}{.35\textwidth}
  {\rmfamily\fontsize{28}{31}\selectfont \QSLPairs}\ candidate pairs\par\vspace{2mm}
  {\small \textbf{100} pairs in the original selection}\par\vspace{2mm}
  {\small Fully scored by two judges (Opus 5 and gpt-5.6).\\Global rank correlation: \textbf{0.740}.}\par\vspace{3mm}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{.27\linewidth}\includegraphics[width=\linewidth]{\assetdir/qsl-paper.png}\end{column}
    \begin{column}{.68\linewidth}{\fontsize{8.5}{11}\selectfont\rmfamily
...
\begin{frame}[t]{How much of creativity is connexion?}
% \kicker{From a seminar question to a change in scientific work}
\vspace{2mm}
% {
% \rmfamily\fontsize{22}{27}\selectfont
If agents can make the connections and also investigate what follows\ldots
% }
\par\vspace{5mm}
{\large What are the other things humans do in science?}\par\vspace{2mm}
{\color{Teal}\large Choosing questions. Inventing concepts. Judging significance. Apply for funding.}\par\vspace{5mm}
% \takeaway{\Large How much longer before all mathematicians are agents?}
%
{\small Can agents also help choose which corpora to bring together?}\par\vspace{2mm}
% \smallprint{Discussion, not a forecast. Most mathematical work being agent-produced would not imply the disappearance of human mathematicians.}
