...
def prepare():
    ...
    assert [p['pair_id'] for p in selection['pairs'] if p['pair_id'] in PAIRS] == PAIRS
    for pair in PAIRS:
        state = json.loads((REPO / 'threads' / pair / 'paper' / 'paper.json').read_text())
        assert state['status'].upper() == 'ACCEPTED'
    selection.update(pairs=[p for p in selection['pairs'] if p['pair_id'] in PAIRS], n_selected=2)
    ...
    write_json(PILOT / 'manifest.json', {
        ...
        'previous_pilot_weekly_remaining_percent': 97,
