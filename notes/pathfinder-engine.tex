...
% --- Fonts: Charter for text and maths, Source Sans for structure, Inconsolata for code
\usepackage[T1]{fontenc}
\usepackage{XCharter}
\usepackage[xcharter,vvarbb]{newtxmath}
\usepackage[semibold,scale=0.98]{sourcesanspro}
\usepackage[varqu,varl,scaled=0.96]{zi4}
\usepackage[protrusion=true,expansion=true,tracking=smallcaps]{microtype}
\SetTracking{encoding=*,shape=sc}{40}
\linespread{1.07}

% --- Page: a 138mm measure (about 70 characters of Charter at 11pt)
\usepackage[a4paper,textwidth=138mm,hmarginratio=1:1,top=30mm,bottom=36mm,
  headsep=10mm,footskip=14mm]{geometry}
...
% --- Colour: one accent, the teal of the campaign heatmap
\usepackage{xcolor}
\definecolor{accent}{RGB}{21,102,121}
\definecolor{accentdark}{RGB}{14,70,84}
\definecolor{ink}{RGB}{40,40,40}
\definecolor{muted}{RGB}{110,110,110}
\definecolor{panel}{RGB}{242,247,248}
\definecolor{stateFill}{RGB}{240,245,246}
\definecolor{termFill}{RGB}{206,229,232}
\definecolor{pauseFill}{RGB}{250,236,214}
\definecolor{blockFill}{RGB}{246,221,218}
...
\usepackage[font={small},labelfont={sf,bf,color=accentdark},labelsep=quad,
  justification=justified,singlelinecheck=true,skip=8pt]{caption}

% --- Headings: Source Sans semibold, numbers in the accent colour
\newcommand{\headfont}{\sffamily\bfseries\hyphenpenalty=10000\exhyphenpenalty=10000}
\titleformat{\section}
  {\headfont\Large\color{ink}}
  {\llap{\color{accent}\thesection\hspace{0.7em}}}{0pt}{#1}
\titleformat{name=\section,numberless}
  {\headfont\Large\color{ink}}{}{0pt}{#1}
\titleformat{\subsection}
  {\headfont\large\color{ink}}
  {\llap{\color{accent}\thesubsection\hspace{0.7em}}}{0pt}{#1}
\titleformat{\paragraph}[runin]
  {\headfont\color{accentdark}}{}{0pt}{\mbox{#1}}
\titlespacing*{\section}{0pt}{2.4\baselineskip plus 0.6\baselineskip minus 0.3\baselineskip}{0.9\baselineskip}
\titlespacing*{\subsection}{0pt}{1.6\baselineskip plus 0.4\baselineskip minus 0.2\baselineskip}{0.6\baselineskip}
\titlespacing*{\paragraph}{0pt}{1.0\baselineskip plus 0.3\baselineskip minus 0.1\baselineskip}{0.6em}

% --- Running heads: title left, current section right, folio in the footer
\pagestyle{fancy}
\fancyhf{}
\renewcommand{\headrulewidth}{0pt}
\fancyhead[L]{\sffamily\footnotesize\color{muted}The Pathfinder engine}
\fancyhead[R]{\sffamily\footnotesize\color{muted}\nouppercase{\leftmark}}
\fancyfoot[C]{\sffamily\footnotesize\color{muted}\thepage}
\renewcommand{\sectionmark}[1]{\markboth{#1}{}}
\fancypagestyle{plain}{\fancyhf{}\fancyfoot[C]{\sffamily\footnotesize\color{muted}\thepage}}

% --- Lists, floats, tables
\setlist{itemsep=0.25\baselineskip,topsep=0.5\baselineskip,parsep=0pt}
\setlist[itemize]{label={\color{accent}\textbullet},leftmargin=1.4em}
\setlist[enumerate]{label={\sffamily\bfseries\color{accent}\arabic*},leftmargin=1.6em}
...
% --- Diagrams: natural size, allowed to extend 10mm into each margin
\newenvironment{widefigure}
  {\par\bigskip\noindent\begin{lrbox}{0}}
  {\end{lrbox}\makebox[\linewidth][c]{\usebox{0}}%
   \typeout{PFWIDE \the\wd0}\par\smallskip}
\newcommand{\guard}[1]{{\scriptsize\color{muted}#1}}
\newcommand{\diagramnote}[1]{{\centering\small\color{muted}#1\par}\bigskip}
\tikzset{
  pf/.style={x=1cm,y=1cm,every node/.style={font=\footnotesize,text=ink},
    >={Stealth[length=2mm,width=1.6mm]}},
  state/.style={draw=accent!75!black,line width=0.5pt,rounded corners=2.5pt,
    minimum height=6.5mm,inner xsep=5pt,fill=stateFill},
  term/.style={state,fill=termFill},
  pause/.style={state,fill=pauseFill,draw=orange!55!black},
  blocked/.style={state,fill=blockFill,draw=red!55!black},
  edge/.style={->,line width=0.6pt,draw=ink!85},
  lab/.style={font=\scriptsize,fill=white,inner sep=1.5pt,text=ink}}

% --- Solid extensible arrows: newtxmath's relbar pieces leave visible gaps
\makeatletter
\def\rightarrowfill@#1{\m@th\setbox\z@\hbox{$#1\rightarrow$}%
  $#1\mkern2mu\leaders\hbox{\vrule height \dimexpr\fontdimen22\textfont2+0.24pt\relax
    depth -\dimexpr\fontdimen22\textfont2-0.24pt\relax width 1pt}\hfill
  \kern-0.8\wd\z@\copy\z@$}
\makeatother

% --- Pair descriptions (quote): a tinted panel with an accent rule
\tcbset{pairpanel/.style={enhanced,breakable,frame hidden,boxrule=0pt,
  sharp corners,colback=panel,borderline west={1.6pt}{0pt}{accent},
  left=1.8em,right=0.8em,top=0.55\baselineskip,bottom=0.45\baselineskip,
  before skip=0.7\baselineskip,after skip=0.9\baselineskip,
  parbox=false,fontupper=\small,
  before upper={\setlength{\parindent}{0pt}\setlength{\parskip}{0.45\baselineskip}}}}
\renewenvironment{quote}{\tcolorbox[pairpanel]}{\endtcolorbox}
% q and p hang to the left of the paper they label
\newcommand{\pl}[1]{\noindent\llap{\makebox[1.5em][l]{\color{accent}\(#1\)}}}
\newcommand{\pair}[1]{\mbox{#1}}

% --- Footnotes
\renewcommand{\footnoterule}{\vspace*{-3pt}{\color{muted}\hrule width 3em height 0.4pt}\vspace*{2.6pt}}
...
% --- Links
\usepackage[colorlinks=true,linkcolor=accentdark,citecolor=accentdark,
  urlcolor=accent,pdftitle={The Pathfinder engine},
  pdfauthor={Vincent Danos, Elham Kashefi, William Waites}]{hyperref}
\urlstyle{tt}
...
\renewcommand{\maketitle}{%
  ...
  \vspace*{6mm}%
  {\sffamily\footnotesize\bfseries\color{accent}\MakeUppercase{\pf@kicker}\par}%
  \vspace{3mm}%
  {\headfont\fontsize{26}{30}\selectfont\color{ink}\@title\par}%
  \vspace{2.5mm}%
  {\ttfamily\small\color{accent}\href{https://\pf@repo}{\pf@repo}\par}%
  \vspace{7mm}%
  {\large\@author\par}%
  \vspace{1.5mm}%
  {\small\color{muted}\pf@affil\par}%
  \vspace{1.5mm}%
  {\small\color{muted}\@date\par}%
  \endgroup
  \vspace{9mm}}
\newcommand{\kicker}[1]{\def\pf@kicker{#1}}
\newcommand{\repo}[1]{\def\pf@repo{#1}}
\newcommand{\affiliations}[1]{\def\pf@affil{#1}}
\renewenvironment{abstract}
  {\par\noindent{\color{accent}\rule{\linewidth}{0.5pt}}\par\vspace{0.6\baselineskip}%
   \noindent{\headfont\small\color{accentdark}Abstract}\par\vspace{0.3\baselineskip}%
...
% --- Bibliography: smaller, with a little air between entries
\makeatletter
\let\pf@oldbib\thebibliography
\renewcommand{\thebibliography}[1]{\pf@oldbib{#1}\small\setlength{\itemsep}{0.3\baselineskip}}
\makeatother

\newcommand{\sref}[1]{\textsuperscript{#1}}

\kicker{Specification note}
\title{The Pathfinder engine}
\repo{github.com/vd1/pathfinder}
\author{Vincent Danos\sref{1}\quad Elham Kashefi\sref{2,3}\quad William Waites\sref{3}}
\affiliations{\sref{1}CNRS, ENS\qquad \sref{2}CNRS, LIP6\qquad \sref{3}University of Edinburgh}
...
\begin{abstract}
Pathfinder is an ``AI for science'' protocol aiming a creating new
research ideas at scale by searching the
scientific literature. Its engine scans and scores pairs of papers from selected
corpora for potential connexions. Top scoring pairs are then given to AI agents
to produce a research note, which is then submitted to an agent verifier who
decides whether to elaborate the note into a publication output for external
assessment. No human is required during the process once the initial corpora is
defined.%
% score threshold for eligibility are
%  and attempts at developing the most promising ones into publishable notes
% which human experts can evaluate.
\footnote{Eventually, in a world where agents dominate the scientific
throughput, a publication-looking output may not be the better format; but that
seems appropriate at the moment for a hybrid AI/human led research process.}

The purpose of this note is to describe the Pathfinder engine as a state
transition system at a level of details suitable for human and AI readers, so
they can jointly and easily re-implement it, should they wish to, and adjust it
...
models), looks for pairs \((q,p)\in Q\times P\) with a scientific
connexion which seems interesting enough to try to elaborate it as a publishable note.
Ideally, the final output satisfies the usual constraints of correctness, novelty,
relevance, readability, and exposition of related work.

% The pair is supplemented
% with a seed note describing the putative connexion, supplied by the agent that
% scanned and scored the pair. If the pair shortlisted, it is passed on to a research
% protocol which if successful may produce a publishable scientific contribution.
% The seed is a starting point, not a claim the researcher agents should support.

Note that Pathfinder is primarily concerned with finding and exploring connexions. In the
course of exploring a connexion agents may state and prove propositions, but one is not
given in advance a conjecture to prove/dis-prove. The idea of generating new
ideas by exploring connexions is not new. Swanson's \emph{undiscovered public
knowledge} is a direct precedent for this motivation: findings may already be
published in separate literatures, yet what they imply together remains
unnoticed until someone brings them into contact \cite{swanson1986}. The
missing step is not necessarily a new observation, but recognising and
investigating a connexion between existing findings. Pathfinder seeks to make
that search systematic.

Gentner's account of analogy describes a more specific way in which a
connexion can work \cite{gentner1983}. Two settings may involve very different
objects but share a pattern of relationships. Recognising that pattern can
suggest transferring an argument or mechanism from one setting to the other;
similar terminology alone is not enough. Analogy is therefore a possible
route to a Pathfinder seed, not a description of every seed. A paper may
instead provide a direct application of another's theorem, expose a failure
of its assumptions, or suggest how to repair its formulation. Swanson
motivates the broader search for knowledge between literatures; Gentner helps
explain a particular form of reasoning within it. In each case, the research
phase must establish what the proposed connexion actually supports.

Closer computational precedents include Krenn and Zeilinger's
\emph{SemNet}~\cite{krenn2020semnet}, which suggests new links between
scientific concepts in a network extracted from the literature, and Gu and
Krenn's \emph{SciMuse}~\cite{gu2025scimuse}, which combines a knowledge graph
with a language model to propose research ideas, evaluated by experts for
interest. Pathfinder starts instead from pairs of papers and asks what might
follow from bringing their specific claims or methods together. Its aim is
also to investigate the proposed connexion, not only suggest it: the research
phase may establish a conditional result, expose an obstruction, or conclude
that further evidence is needed. The distinction is therefore in the starting
unit and the subsequent investigation, not in the general idea of using AI
to find scientific connexions.

What is new is that AI agents
are now available that can plausibly stand in for human experts and search this
space at scale. Whether the systematic search for connexions leads to useful
scientific knowledge can now be tested. Pathfinder is a first step in this
direction.

% \subsection{Object of this note}
The goal of this note is to specify the underlying protocol at a level sufficient for an
agent or a human programmer to reimplement it, without prescribing
...
we picked the ten most recent papers (at the time of query, Sep 15, 2026)
...
The scanner agent (Sonnet 5) scored each of the $100$ pairs from $0$ to $100$
for feasibility and scientific-gain, and proposed a seed connexion. It did so
being only given titles and abstracts (thereby keeping contexts small). The
ranking score is the product.
% There were $99$ parseable scored pairs: the response for \pair{Q1P7} could not
...
\IfFileExists{notes/figures/2026-09-14-campaign-heatmap.tex}
  {\input{notes/figures/2026-09-14-campaign-heatmap.tex}}
  {\input{figures/2026-09-14-campaign-heatmap.tex}} \caption{100 pairs scanned,
  14 retained for research (black outline),
  5 accepted as papers (starred).
...
\bottomrule
\end{tabular}
\caption{Root-campaign model usage.}
...
\begin{table}[htbp]
\centering
\begin{tabularx}{0.72\linewidth}{@{}Xr@{}}
\toprule
...
The highest-scoring pair, \pair{Q4P10}, did not reach \st{draft}: a generic incentive
inequality was not enough to establish the proposed connexion to the energy
simulation. Conversely, the retained low-scoring \pair{Q1P2} did produce an accepted
manuscript (a potential false negative for the scanning phase). The scan ranking
...
\pair{Q3P10} required unavailable market experiments. For \pair{Q3P3}, the researchers
identified a missing comparison in \pair{P3}: does adversarial review outperform
...
to reproduce and extend \pair{P3}'s experiments. A pause
...
\midrule
\pair{Q1P2} & Learning-augmented algorithms and concept-guided exploration &
Room-level calibration certifies the non-abstained decisions of a first
wall-distance gate. \\
\pair{Q1P6} & Learning-augmented algorithms and MARS-RA credit assignment & A
fixed-population reward ledger repairs potential shaping under changing
coalitions. \\
\pair{Q4P6} & Mechanism design for alignment and MARS-RA credit assignment & A
conditional interface recovers contribution rankings from noisy peer signals. \\
\pair{Q7P1} & Stochastic-stability mechanism design and distributed CVaR optimisation
& A uniform surrogate-error bound controls static welfare loss and conditional
participation loss. \\
\pair{Q7P7} & Stochastic-stability mechanism design and carbon-response forecasting &
...
Consider the pair:
\begin{quote}
\pl{q}\href{https://arxiv.org/abs/2609.01595}{\file{arXiv:2609.01595}},
...
\pl{p}\href{https://arxiv.org/abs/2607.27967}{\file{arXiv:2607.27967}},
...
\begin{quote}
\pl{q}\href{https://arxiv.org/abs/2608.29130}{\file{arXiv:2608.29130}}, \emph{A
...
\pl{p}\href{https://arxiv.org/abs/2609.04460}{\file{arXiv:2609.04460}},
...
\begin{quote}
\pl{q}\href{https://arxiv.org/abs/2609.04787}{\file{arXiv:2609.04787}},
...
\pl{p}\href{https://arxiv.org/abs/2608.23650}{\file{arXiv:2608.23650}},
...
This pair also illustrates how the scanner can overlook a promising connexion.
Sonnet 5 assigned feasibility $30$ and gain $20$, giving $600$, below the
shortlist threshold of $1500$. On 21 September, a fresh Astra instance
(\file{gpt-6-astra}) received only the same titles and abstracts and the current
scanning rubric, without the earlier score, seed or research outcome. It
assigned feasibility $64$ and gain $58$, giving $3712$: the pair would have
passed the threshold. Astra proposed combining the robot's room-based
predictions with a fallback exploration strategy, then asking whether correct
predictions save work while incorrect ones cause only a bounded extra cost.
It also identified a difficulty specific to this setting: exploration changes
the information on which subsequent predictions depend.

In this instance, Astra recognised substantially more promise in the connexion.
Its proposal was broader than the selective perception guarantee developed
below; it did not independently recover or validate that result. The rescore
is an encouraging example of recovering a candidate missed by the original
threshold, rather than evidence of generally superior detection from a single
comparison.\footnote{The rescore used a fresh agent instructed to use no tools,
not the original scanner transport. The current prompt was not checked against
a historical snapshot. Inputs, response and provenance are saved in
\file{experiments/2026-09-21-174126-astra-blind-Q1P2/}.}
...
\begin{quote}
\pl{q}\href{https://arxiv.org/abs/2608.29130}{\file{arXiv:2608.29130}},
...
The original \pair{Q7P1} proposal sought to combine distributed risk-averse
...
why those distinctions matter. The \pair{Q7P1} follow-up identified an obstruction to
...
\pl{p}\href{https://arxiv.org/abs/2607.26560}{\file{arXiv:2607.26560}},
...
later \pair{Q7P1} obstruction therefore raises a dependency concern for that
conditional implementation claim too; this is an implication of the follow-up,
not a completed reassessment of \pair{Q7P7}.
...
\begin{widefigure}
\begin{tikzpicture}[pf]
  \node[align=center] (input) at (0,0) {input\\\((q,p,c)\)};
  \node[state] (peers) at (2.2,0) {\st{research}};
  \node[state] (cons) at (6.3,0) {\st{consolidate}};
  \node[state] (ver) at (9.8,0) {\st{verify}};
  \node[pause,align=center,anchor=west] (poi) at (11.5,1.7)
    {\st{pause-on-iterate}\\[-1pt]\guard{\st{iterate}, rounds out}};
  \node[pause,align=center,anchor=west] (por) at (11.5,0.45)
    {\st{pause-on-revise}\\[-1pt]\guard{\st{revise}, repairs out}};
  \node[term,anchor=west] (draft) at (11.5,-0.6) {\st{draft}};
  \node[pause,anchor=west] (pause) at (11.5,-1.5) {\st{pause}};
  \draw[edge] (input) -- (peers);
  \draw[edge] (peers) -- node[lab,above,align=center] {all ready, or\\allowance out} (cons);
  \draw[edge] (cons) -- node[lab,above] {note written} (ver);
  \draw[edge] (ver.north) to[out=115,in=65,looseness=0.5]
    node[lab,above] {\st{iterate}, rounds left} (peers.north);
  \draw[edge] (ver.south) to[out=245,in=295,looseness=0.8]
    node[lab,below] {\st{revise}, repairs left} (cons.south);
  \draw[line width=0.6pt,draw=ink!85] (ver.east) -- (10.75,0);
  \fill[ink!85] (10.75,0) circle (1.1pt);
  \foreach \t in {poi,por,draft,pause}
    \draw[edge] (10.75,0) |- (\t.west);
  \draw[edge] (peers.south) -- (2.2,-2.35)
    -- node[lab,above,pos=0.45] {no substantive ledger entry} (pause.south |- 0,-2.35) -- (pause.south);
\end{tikzpicture}
\end{widefigure}
\diagramnote{Research phase. The input \((q,p,c)\) carries an optional seed connexion \(c\).}
...
\begin{widefigure}
\begin{tikzpicture}[pf]
  \node[term] (draft) at (0,0) {\st{draft}};
  \node[state] (author) at (2.9,0) {\st{author}};
  \node[state] (review) at (7.6,0) {\st{review}};
  \node[term] (accepted) at (12.4,0) {\st{accepted}};
  \node[pause] (paused) at (7.6,-1.6) {\st{pause-on-amend}};
  \draw[edge] (draft) -- node[lab,above] {request} (author);
  \draw[edge] (author) -- node[lab,above] {manuscript checked} (review);
  \draw[edge] (review) -- node[lab,above,align=center] {\st{accept},\\build succeeds} (accepted);
  \draw[edge] (review.north) to[out=120,in=60,looseness=0.8]
    node[lab,above] {\st{amend}, rounds left} (author.north);
  \draw[edge] (review) -- node[lab,right] {\st{amend}, rounds out} (paused);
  \node[state,align=center] (terminal) at (0.35,-3.6) {terminal research\\outcome};
  \node[state] (editor) at (5.2,-3.6) {\st{editor}};
  \node[term] (readable) at (11.6,-3.6) {\st{readable account}};
  \node[blocked] (blocked) at (11.6,-5.0) {\st{blocked}};
  \draw[edge] (terminal) -- node[lab,above] {request} (editor);
  \draw[edge] (editor) -- node[lab,above] {build and checks pass} (readable);
  \draw[edge] ([xshift=-2.5mm]editor.north) to[out=115,in=65,looseness=5]
    node[lab,above] {check failure, retry left} ([xshift=2.5mm]editor.north);
  \draw[edge] ([xshift=3mm]editor.south) |- node[lab,above,pos=0.72] {check failure, attempts out} (blocked.west);
\end{tikzpicture}
\end{widefigure}
\diagramnote{Editing phase: the paper branch (top) and the readable-account branch (bottom).}
...
Judges also receive the note being assessed. What one call can reuse from an
earlier one is therefore the leading part only, the papers and the ledger
entries already present; the new entries, the note and the instruction after
them change. An implementation should preserve
...
Every attempted call appends a receipt, specified below. The admission guard
compares recorded expenditure, a per-call estimate for every call that
established a session and whose cost is unknown, and a reservation for work in
flight against the campaign budget. This is a
...
\paragraph{Call receipts.}
The receipts are the campaign's only record of what it consumed, and every
statement about cost or time rests on them. They are therefore part of the
specification. The runner appends one receipt for every call it attempts,
including an agent that could not be launched, a call that never established a
session, and a call ended at its deadline. This covers the attempts the runner
handles; it is not a promise of durable accounting across an abrupt crash of the
runner itself.

A receipt records the time, the thread, the stage and the role, the backend and
the model requested, the elapsed time, and an outcome: completed, error,
timeout, no session, or launch failed. It records the provider's usage counters
as the provider reported them, with the backend named, because providers count
differently: one includes cached tokens in its input total where another reports
them apart, and reasoning may or may not be included in output. A breakdown into
uncached input, cache reads, cache writes and billed output is derived later
from the raw counters and their documented meaning; it is not substituted for
them.

What was not reported stays unknown. A missing counter is recorded as absent,
never as zero, while a reported zero remains zero. Counters that did arrive are
kept on a failed or interrupted call as well, where the outcome tells the reader
that they may be partial: such a call can consume resources without saying how
much. A cost, in US dollars, carries its basis. It is either the amount the
provider reported, or an amount priced from reported counters at rates the
receipt retains, which is an approximation and not a reconstructed provider
charge. When neither is possible the cost is unknown. An amount computed for a
subscription is an equivalent, not an additional bill. No receipt holds an
estimate: the guard's caution about calls of unknown cost belongs to the guard,
which does not charge a call that never reached a session, and a display of expenditure states the known cost and, separately, how many
calls have unknown cost. Repricing under another tariff is an analysis over
receipts and adds nothing to them.

The guarantee is this: enough is recorded to reconstruct what a campaign
consumed and, where the provider reports enough, to reprice it, and what cannot
be known is visibly unknown. The reference receipts do not identify retries;
that identity must not be inferred from adjacent receipts, since an ordinary
later call can carry the same thread, stage and role.
...
decision has not been made. The first operational failure observed by the research
scheduler stops campaign admission. Active work drains before a nonzero exit;
there is no automatic health-probe loop. Editor failures propagate as operational
failures, and terminal research with an unfinished account remains pending for
an explicit restart. A call that starts but times
...
An external supervising agent can audit the runner every five minutes through
\texttt{pathfinder health --json}. The read-only snapshot separates scheduler
heartbeat from completed-investigation progress and lists active call identities,
owner and child process identifiers, nominal deadlines, and recent failures.
Overdue work or a stale heartbeat warrants investigation; neither proves a
deadlock. Existing transport startup and cleanup can exceed the nominal call
allowance. An exclusive operating-system lock prevents concurrent research
runners, and restart refuses recorded calls whose owner or child process remains
alive. Process identity and surviving descendants still require inspection.
The supervising agent owns scheduling, diagnosis, and an incident ledger of
evidence, action, and subsequent progress. The package does not itself install
that recurring audit or guarantee delivery to an agent.
...
\[
 ...
 \pi=\st{accepted}
    &;\Longrightarrow;\begin{array}[t]{@{}l@{}}
       \text{an accepting review and successful build}\\
       \text{identify the same manuscript version.}
     \end{array}
 \end{aligned}
\]
...
\item An operational failure setting the health marker and stopping admission
without probing, even when another investigation succeeds.
\item Terminal research resuming unfinished editing without repeating research.
\item Read-only health snapshots distinguishing stale heartbeats, missing runner
processes, and overdue calls, with exclusive ownership released on process death.
\item An agent that cannot be launched, and a call that never establishes a
session, each leaving a receipt with no usage and no cost.
\item A call ended at its deadline leaving a receipt that keeps whatever usage
was reported, and none when none was, with the guard still counting it; and a
missing counter recorded as unknown while a reported zero stays zero.
...
\section{Variability, evaluation and discovery comparison}
\label{app:discovery-recovery}

Repeating an investigation can produce a different connexion (), a different
connexion, or a different verdict. Moreover, judging the resulting science is
itself uncertain. These are separate questions: how variable is the research,
and how reliably can we assess it? The following related work motivates a
proposed experiment, not a further result of the pilot.

\subsection{What existing evaluations measure}

\paragraph{Tasks with assessable answers.}
The starting questions in the studies below come from benchmark instances. There
is no research-seed generation stage to compare with Pathfinder's scan. Their
agents generate candidate answers or action plans, not new research questions.
\emph{More Agents Is All You Need} studies sampling and voting on benchmark
tasks; its sampling-parameter robustness experiments average over twenty
runs~\cite{li2024more}. \emph{Debate or Vote} compares independent voting with
agent discussion against benchmark answers. Its main table reports five-run
variation for the single-agent baseline, but not equivalent error bars for the
multi-agent configurations~\cite{choi2025debate}. The broader comparison by Kim
et al. uses task completion and correctness across agent architectures, with
confidence intervals and robustness analyses~\cite{kim2026scaling}. Wunderlich
et al. compare accuracy at comparable computational cost, reporting sampling
uncertainty over benchmark questions~\cite{wunderlich2026compute}; Tran and
Kiela likewise show why controlling the reasoning budget
matters~\cite{tran2026single}. In contrast, $\tau$-bench explicitly repeats
tasks and distinguishes finding a successful attempt from succeeding
consistently across attempts. Its tasks are constructed from human-designed
domains, policies and annotated user requests, with model assistance for data
generation. Evaluation relies on an annotated target database
state~\cite{yao2024tau}, an advantage unavailable for an undiscovered result.

\paragraph{Classifying scientific-research systems.}
The closer comparisons concern systems that propose research rather than
solve supplied benchmark tasks. We distinguish how the starting idea is
generated, the protocol that develops it, the output produced, and how that
output is evaluated. In describing the starting idea, supplying a broad topic
is different from supplying the hypothesis to investigate. Likewise, review
inside the generation protocol is distinct from evaluation of the system by
its authors or independent readers.

\paragraph{VirSci~\cite{su2025virsci}.}
\emph{Seed generation:} agents represent scientists with publication-based
profiles. A sampled leader recruits collaborators, and the team chooses a
topic through discussion, then retrieves relevant papers and proposes ideas.
\emph{Generation protocol:} agents take turns proposing or revising ideas and
experimental plans, retain highly self-rated candidates, and vote on novelty
against retrieved literature. They iteratively refine the selected idea's
abstract. \emph{Output:} a research abstract describing proposed work and
expected results, not an executed study. \emph{Evaluation:} literature-similarity
metrics, agent reviews and human reviews. Scenarios are repeated twenty times;
a 200-abstract validation uses three human assessments and three agent
assessments per abstract.

\paragraph{Si et al.~\cite{si2024ideas}.}
\emph{Seed generation:} a human-supplied topic guides literature retrieval.
The agent generates 4,000 candidate ideas per topic, using example proposals
and, for some generations, retrieved titles and abstracts.
\emph{Generation protocol:} generate many candidates, remove near-duplicates,
expand ideas into structured project proposals, and rank them through pairwise
agent comparisons. A separate condition uses human reranking.
\emph{Output:} research proposals, not completed investigations.
\emph{Evaluation:} blind expert comparison with human-written proposals,
using novelty, feasibility, expected effectiveness and related criteria;
the analysis accounts for topic, idea and reviewer variation and measures
reviewer disagreement.

\paragraph{The AI Scientist-v2~\cite{yamada2025scientist}.}
\emph{Seed generation:} humans supply broad topical prompts, chosen for the
workshop's scope in the reported experiment. The system formulates hypotheses
and experimental plans while consulting literature through Semantic Scholar.
The topic is human-provided; the specific research idea is agent-generated.
\emph{Generation protocol:} an experiment manager coordinates code generation,
execution and refinement through tree search, progressing from a working
prototype through tuning, the research agenda and ablations. Selected
experiments are replicated; the system generates figures and a manuscript,
with model feedback and revision. No supplied experimental code template is
required. \emph{Output:} full experimental papers.
\emph{Evaluation:} internal model feedback, author inspection and external
workshop peer review. The authors submitted three manuscripts and report that
one met the acceptance threshold.

\paragraph{Pathfinder.}
\emph{Seed generation:} humans select corpora $Q$ and $P$. The scanner examines
pairs in $Q\times P$, proposes connexions from titles and abstracts, and scores
whether they warrant investigation. The specific question emerges from this
encounter rather than from a supplied topic to elaborate.
\emph{Generation protocol:} researchers investigate with a shared ledger;
consolidation and verification lead to bounded cycles of further research or
repair, or a terminal outcome. Researchers may abandon or reformulate the
seed. Editing follows research, producing a reviewed manuscript when warranted
and a readable account of terminal research outcomes.
\emph{Output:} a research record and, where justified, a pre-publication paper;
paused investigations also yield an account of what was learned.
\emph{Evaluation:} internal agent verification and manuscript review, plus
build checks. The pilot inventories resulting claims but does not establish
an independently validated discovery rate. The experiments proposed below
would compare scientific contributions across protocols.

Generating ideas and producing papers both have precedents. Pathfinder's
distinguishing design choice in this comparison is its systematic use of
cross-corpus pairings to generate and select questions, making the candidate
space and its coverage explicit. Other systems also use prior literature;
the distinction is not simply ``ideas from papers''. This identifies a design
difference, not a claim of priority over all literature-based discovery
methods. In every case, assessment of scientific claims remains necessary
beyond the output format or an acceptance decision.

\subsection{Independent research runs, compared afterwards}

Rather than ask reviewers to give every output an overall quality score, we
could compare the specific contributions found by different protocols. The
labels would be constructed afterwards from all protocols' findings, not
fixed in advance or supplied by the current protocol alone. This experiment
stops after the research phase, including consolidation and verification:
neither the manuscript nor the readable-account editing branch is needed.

\begin{enumerate}
\item \textbf{Run independently.} Repeat each protocol on fixed example pairs,
initially \pair{Q3P3} and \pair{Q4P6}. Compare the current protocol with alternatives
such as a single researcher, independent researchers followed by consolidation,
and interacting researchers. Fix the papers, optional seed, model, tool access,
resource allowances and number of runs before examining the outcomes. Retain
the same final verification procedure and include coordination and verification
in the budget. Keep every research record, including pauses and operational
failures. No run receives another run's findings; access to published pilot
outputs must also be controlled and documented.

\item \textbf{Construct the labels.} Pool the completed research records and
have assessors, blind to the producing protocol, identify and check their
contributions. Each label specifies a claim, its assumptions, its supporting
argument or experiment, and the criterion for an equivalent finding. Merge
duplicate formulations without merging substantively different assumptions
or guarantees. Fix the labelling rules in advance, although the labels
themselves emerge afterwards. Double-assess a subset to measure disagreement.

\item \textbf{Compare the findings.} Map each run to its checked labels, then
report shared and protocol-specific contributions, their frequency across
repeated runs, and the number of distinct contributions found at comparable
total expenditure. Retain the evidence for each attribution: mentioning a
conclusion is not equivalent to establishing it.
\end{enumerate}

For a pair $x$, let $L_{\pi,j}(x)$ be the checked labels found by run $j$ of
protocol $\pi$, and define the pooled inventory and each run's coverage by
\[
  L_x=\bigcup_{\pi,j}L_{\pi,j}(x),
  \qquad
  \operatorname{coverage}_{\pi,j}(x)
  =100\,\frac{|L_{\pi,j}(x)|}{|L_x|}
  \quad\text{when }|L_x|>0.
\]
If no checked contribution is found for a pair, report that outcome rather
than an undefined percentage. Give per-pair coverage distributions and
per-label frequencies, not just the best run. A launch without a research
output contributes no labels and is also recorded as an operational failure.
Report actual tokens, cost and elapsed time alongside the scientific outcomes.

The examples in Section~\ref{sec:pilot} and the talk's inventory illustrate
possible labels: the conditional judge-free ranking construction for \pair{Q4P6},
or the identification and design of a missing matched-budget control for
\pair{Q3P3}. The latter is an experimental-design contribution, not evidence that
the critic is ineffective. Keep proposed tests distinct from proved or
experimentally established results. Supported obstructions may count even
when a run pauses; internal \st{draft} status alone establishes neither
validity nor novelty.

\paragraph{What this comparison would establish.}
Coverage measures the share of the pooled findings, not the share of all
possible discoveries. The inventory depends on the protocols and runs sampled;
the comparison is therefore exploratory, not a held-out test against an
independent answer key. It allows a new contribution from any protocol to
enter on equal terms, while distinguishing repeated access to familiar
findings from complementary discoveries. A protocol-specific label is not
automatically more valuable, and counts depend on the agreed granularity.
Record unsupported claims separately so that high coverage cannot conceal
unreliable research. The inventory could subsequently be frozen for a fresh
recovery experiment. Neither comparison establishes generalisation beyond
the tested pairs, nor does evaluation before editing measure manuscript
quality. It makes scientific assessment more specific, not unnecessary.

\subsection{Virtual conferences and successive generations}

A more ambitious experiment would turn each campaign into a virtual
conference. Starting from the same source corpora, each protocol produces a
batch of research papers, submits them to a common review procedure, and
returns admitted papers to the pool of candidates for subsequent scans.
Further generations can then investigate connexions involving earlier
generated work. Fix an admission and pairing rule in advance: which papers
enter $Q$, $P$, or both, and whether generated papers may be paired with
each other. Preserve source versions and ancestry throughout. Admission is
permission to investigate a paper further, not certification that its claims
are true.

This asks whether a protocol can sustain cumulative research, rather than
recover a fixed inventory. For example, a later paper might relax the
assumptions of the \pair{Q4P6} ranking construction, establish its finite-sample
requirements, or find a counterexample. These are possible directions, not
results already obtained. A contribution must add something beyond both the
initial literature and its generated predecessors; a new pairing, title or
restatement is not enough. Corrections and supported negative results also
count, but an unsupported premise inherited from an earlier paper must not
silently become an established fact.

Compare separate protocol lineages from the same initial corpora, with the
same review procedure and resource allowance per generation, and repeat each
lineage independently. Count distinct, independently checked contributions
and their evidential types, alongside invalidated claims, costs and human
assessment effort. Plot cumulative contributions against total expenditure,
not merely against generation number or paper count. Keep the assessors
separate from the producing agents; agent review may screen candidates, but
human audits and claim-specific checks remain necessary. If only a sample is
audited, report that sampling and its uncertainty rather than describing all
admitted papers as validated.

The evolving corpora are themselves an outcome: a successful discovery may
make later progress easier. Such a comparison measures a whole research
lineage, including selection and accumulated opportunities. To distinguish
this effect from a better research procedure on the same inputs, periodically
test the protocols on a common frozen pool. Keep this longitudinal evaluation
separate from the single-generation comparison: its aim is to measure additional
supported ideas, not agreement with an ever-expanding answer key.

\bibliographystyle{unsrt}
\IfFileExists{notes/pathfinder-engine.bib}
  {\bibliography{notes/pathfinder-engine}}
  {\bibliography{pathfinder-engine}}
