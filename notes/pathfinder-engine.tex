...
The starting questions in the studies by Li et al., Choi et al., Kim et al.,
Wunderlich et al., and Tran and Kiela below come from benchmark instances.
Their agents generate candidate answers or action plans, not new research
questions. There is therefore no research-seed generation stage to compare
with Pathfinder's scan.
...
variation for the single-agent baseline, but not equivalent error bars for
the multi-agent configurations~\cite{choi2025debate}. The broader comparison
by Kim et al. uses task completion and correctness across agent architectures,
with confidence intervals and robustness analyses~\cite{kim2026scaling}.
Wunderlich et al. compare accuracy at comparable computational cost, reporting
sampling uncertainty over benchmark questions~\cite{wunderlich2026compute};
Tran and Kiela likewise show why controlling the reasoning budget
matters~\cite{tran2026single}. In contrast, $\tau$-bench explicitly repeats tasks and distinguishes
finding a successful attempt from succeeding consistently across attempts.
Its tasks are constructed from human-designed domains, policies and annotated
user requests, with model assistance for data generation. Evaluation relies
on an annotated target database state~\cite{yao2024tau}, an advantage
unavailable for an undiscovered result.
